...
class NN:
    def __init__(self):
        ...
        self.BATCH_SIZE = 10000  # 一个训练batch中的训练数据个数

        # 神经网络相关参数
        self.LAYER_NODE = [50, 50, 50, 50]  # 隐藏层节点数。5层，5*50个节点
    ...
    def inference(self, input_tensor):
        ...
        # layer1 = tf.nn.tanh(tf.matmul(input_tensor, self.weights1) + self.biases1)
        # layer2 = tf.nn.tanh(tf.matmul(layer1, self.weights2) + self.biases2)
        # layer1 = tf.nn.relu(tf.matmul(input_tensor, self.weights1) + self.biases1)
        # layer2 = tf.nn.relu(tf.matmul(layer1, self.weights2) + self.biases2)
        # layer1 = tf.nn.sigmoid(tf.matmul(input_tensor, self.weights1) + self.biases1)
        # layer2 = tf.nn.sigmoid(tf.matmul(layer1, self.weights2) + self.biases2)
        # 计算输出层前向传播结果。
        layer = []
    ...
    def train(self):
        ...
        # accuracy = tf.reduce_mean(tf.square(self.obj_f(x, t)))
        ...
        with tf.Session() as sess:
            ...
            for i in range(TRAINING_STEPS):
                ...
                if i % 100 == 0:
                    validate_acc = sess.run(accuracy, feed_dict=train_dict)
                    print('After %d training steps, validation accuracy is %g' % (i, validate_acc))
                    loss_value = sess.run([loss_R, loss], feed_dict=train_dict)
    ...
    def plotting(self):
        ...
        for i in range(100):
            ...
            f = self.sess.run(self.test_f(x, y))
